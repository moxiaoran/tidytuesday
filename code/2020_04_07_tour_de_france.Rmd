...
[TidyTuesday live screencast: Analyzing Tour de France data in R](https://www.youtube.com/watch?v=vT-DElIaKtE)
...
tdf_winners %>%
  ...
  geom_col() +
  labs(y = "",
       title = "What countries won the msot Tour def France winners born in?")

tdf_winners %>%
  count(winner_name, birth_country,sort = T)
...
by_decade <- tdf_winners %>%
  ...
  summarize(winner_age = mean(age),
            winner_height = mean(height, na.rm = T),
            winner_weight = mean(weight, na.rm = T),
...
by_decade %>%
  ...
  expand_limits(y = 0) +
  labs( x= "Decade",
        y = "Average margin of winer (minutes)",
        title = "Tour de France race have been getting closer")
...
by_decade %>%
  ...
  expand_limits(y = 0) +
  labs( x= "Decade",
        y = "Average speed of winner (minutes)",
        title = "Tour de France winner have been getting faster")
...
```
Median life expectancy of a Tour de Franc winner is 77.
...
stages_joined %>%
  ...
  summarize(stages = n(),
            median_percentile = median(percentile, na.rm = T)) %>%
  arrange(desc(stages))
...
stages_joined %>%
  ...
  select(year, rider, points_rank, percentile_first_stage = percentile) %>%
